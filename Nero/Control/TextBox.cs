...
namespace Nero.Control
{
    ...
    public class TextBox : Control
    {
        ...
        /// Cor de fundo
        /// </summary>
        public Color FillColor = new Color(10, 10, 10);

        /// <summary>
        /// Cor de fundo - focus
        /// </summary>
        public Color FillColor_focus = new Color(10, 10, 10);
        ...
        /// Cor da borda
        /// </summary>
        public Color OutlineColor = new Color(100, 100, 100);

        /// <summary>
        /// Cor da borda - focus
        /// </summary>
        public Color OutlineColor_focus = new Color(85, 132, 189);
        ...
        #region Events
        public event HandleCommon OnValidate, OnEnter;
        #endregion
        ...
        void Draw_Multiple(RenderTarget target)
        {
            ...
            // Fundo
            if (HasFocus())
                DrawRectangle(target, gp, Size, FillColor_focus, OutlineThickness, OutlineColor_focus);
            else
                DrawRectangle(target, gp, Size, FillColor, OutlineThickness, OutlineColor);
            DrawShadowRectangle(target, gp - new Vector2(OutlineThickness, OutlineThickness), Size + new Vector2(OutlineThickness, OutlineThickness) * 2,
                Border_Opacity, Border_Scale);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (HasFocus() && Line_SelectIndex == i && s_animation)
                {
                    ...
                    DrawText(render, "|", 12, new Vector2(2 + GetTextWidth(displaytext.Substring(0, Character_CurrentIndex)), 2 + 14 * i), new Color(210, 210, 210));
                }

                DrawText(render, displaytext, 12, new Vector2(2, 2 + 14 * i), new Color(210, 210, 210));
            }

            render.Display();

            var spr = new Sprite(render.Texture);
            spr.Position = gp + new Vector2(1, 1);
            target.Draw(spr);
        ...
        void Draw_Normal(RenderTarget target)
        {
            ...
            if (!HasFocus() && Text.Trim().Length == 0)
            {
                ...
                DrawText(target, pre_Text, 12, gp + new Vector2(X, (Size.y / 2) - 7), new Color(150, 150, 150));
                return;
            }
            ...
            if (HasFocus() && s_animation)
            {
                ...
                DrawText(target, "|", 12, gp + new Vector2(X + GetTextWidth(display.Substring(0, Character_CurrentIndex - diff)), (Size.y / 2) - 7), new Color(210, 210, 210));
            }
            DrawText(target, display, 12, gp + new Vector2(X, (Size.y / 2) - 7), new Color(210, 210, 210));
            if (HasFocus() && Text.Length > 0 && Suggestion.Count > 0) Draw_Suggestion(target);
        }
